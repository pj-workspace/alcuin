...
class ProviderConfig:
    ...
    def model(self, model_id: str) -> ProviderModelConfig | None:
...
class Settings(BaseSettings):
    ...
    operator_api_key: SecretStr | None = Field(default=None, repr=False)
    ...
    provider_mode: str = Field(
        default="responses", pattern="^(responses|chat_completions)$"
    )
    deepseek_api_key: str | None = None
    deepseek_base_url: str = "https://api.deepseek.com"
    deepseek_model: str = "deepseek-v4-flash-vision-exp"
    deepseek_protocol: str = Field(
        default="chat_completions", pattern="^(responses|chat_completions)$"
    )
    context_window_tokens: int = Field(default=131_072, ge=8_192, le=2_000_000)
    context_reserved_output_tokens: int = Field(default=16_384, ge=256, le=131_072)
    ...
    attachment_document_max_bytes: int = Field(
        default=8 * 1024 * 1024,
        ge=64 * 1024,
        le=32 * 1024 * 1024,
    )
    attachment_run_total_max_bytes: int = Field(
        default=20 * 1024 * 1024,
        ge=64 * 1024,
        le=80 * 1024 * 1024,
    )
    attachment_extracted_max_chars: int = Field(
        default=120_000,
        ge=1_000,
        le=500_000,
    )
    attachment_pdf_max_pages: int = Field(default=100, ge=1, le=500)
    attachment_staged_ttl_seconds: int = Field(
        default=24 * 60 * 60,
        ge=60,
        le=7 * 24 * 60 * 60,
    )
    ...
    def model_post_init(self, context: object) -> None:
        del context
        operator_api_key = (
            self.operator_api_key.get_secret_value().strip()
            if self.operator_api_key is not None
            else ""
        )
        if self.is_production and not operator_api_key:
            raise ValueError("ALCUIN_OPERATOR_API_KEY must be configured in production")
        if operator_api_key.startswith("alc1."):
            raise ValueError(
                "ALCUIN_OPERATOR_API_KEY cannot use the reserved embed-token prefix"
            )
        ...
        # Derive smaller defaults for small context windows. Explicit operator
        # budgets are never silently rewritten, including environment values.
        if "context_reserved_output_tokens" not in self.model_fields_set:
            available = self.context_window_tokens - (
                self.context_reserved_tool_tokens
                if "context_reserved_tool_tokens" in self.model_fields_set
                else 0
            )
            self.context_reserved_output_tokens = min(16_384, max(256, available // 4))
        if "context_reserved_tool_tokens" not in self.model_fields_set:
            available = self.context_window_tokens - self.context_reserved_output_tokens
            self.context_reserved_tool_tokens = min(8_192, max(0, available // 4))
        if (
    ...
    def provider(self, provider_id: str) -> ProviderConfig:
        if provider_id == "deepseek":
            ...
            if not any(model.id == self.deepseek_model for model in models):
                models.append(
                    ProviderModelConfig(
                        id=self.deepseek_model,
                        label=self.deepseek_model,
                        tier="custom",
                        input_modalities=("text",),
                        reasoning_efforts=reasoning_efforts,
                    )
            ...
            default_model = next(
                model for model in models if model.id == self.deepseek_model
            )
            return ProviderConfig(
                ...
                input_modalities=default_model.input_modalities,
                models=tuple(models),
            )
        default_model = ProviderModelConfig(
            id=self.openai_model,
            label=self.openai_model,
            tier="standard",
            input_modalities=("text", "image"),
            reasoning_efforts=("none", "low", "medium", "high"),
        )
        ...
        return [
            {
                ...
                "models": [model.public() for model in provider.models],
            }
            for provider in (
