...
export function RunsView({ runs, events, onSelectEvents }: { runs: Run[]; events: ExecutionEvent[]; onSelectEvents: (events: ExecutionEvent[]) => void }) {
  ...
  return <div className="wide-surface runs-surface">
    ...
    <div className="runs-body">
      <section className="run-list-panel"><div className="table-toolbar"><div className="filter-input"><Search size={14} /><input placeholder={t("Search runs…")} /></div><button className="button secondary"><Filter size={13} />{t("Filter")}</button></div><div className="run-table-head"><span>{t("Run")}</span><span>{t("Status")}</span><span>{t("Agent")}</span><span>{t("Started")}</span><span /></div>{runs.map((run) => <button key={run.id} className={clsx("run-table-row", selected === run.id && "active")} onClick={() => void select(run.id)}><span><strong>{run.title ?? run.input}</strong><small>{run.id.slice(0, 18)}</small></span><StatusPill status={run.status} /><span>{run.agent_name ?? "Operations Copilot"}</span><span>{relativeTime(run.created_at, t)}</span><ChevronRight size={14} /></button>)}</section>
