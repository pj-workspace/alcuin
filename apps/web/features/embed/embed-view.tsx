...
import { API_URL, alcuinApi } from "@/shared/lib/api";
...
<alcuin-agent
  ...
  agent-name="${agent?.name ?? "Alcuin Starter"}"
  theme="auto"
...
></alcuin-agent>`, [agent?.name, locale, token]);
  ...
  useEffect(() => {
    if (componentReady && liveElement) {
      liveElement.setContext({
        page: "/workspace/records/REC-104",
        record: { id: "REC-104", status: "active", type: "project" },
      });
    }
  ...
  return <div className="wide-surface embed-surface">
    ...
    <div className="embed-security"><ShieldCheck size={16} /><div><strong>{t("Scoped by design")}</strong><span>{t("Tokens bind workspace, published Agent, allowed origin, actions and expiry.")}</span></div><span className={clsx("session-state", token && "active")}>{token ? <><Check size={12} />{t("Session active")}</> : t("No live token")}</span></div>
    <div className="embed-workspace">
      <section className="code-panel"><header><div><Code2 size={15} /><span>{t("Embed code")}</span></div><button onClick={() => { void navigator.clipboard.writeText(fullSnippet); notify(t("Embed snippet copied")); }}><Copy size={13} />{t("Copy")}</button></header><pre><code>{displaySnippet}</code></pre><div className="code-notes"><div><span>01</span><p>{t("Create sessions server-side. Never expose your workspace credential.")}</p></div><div><span>02</span><p>{t("Pass host context with")} <code>element.setContext(...)</code>.</p></div><div><span>03</span><p>{t("Listen for run, artifact, approval and error custom events.")}</p></div></div><div className="sdk-row"><span>{t("Package")}</span><code>@alcuin/embed</code></div></section>
      <section className="preview-panel"><header><div><span>{t("Live host preview")}</span><small>{t("Host application")} · {t("real Web Component")}</small></div><div className="device-switch"><button className={clsx(device === "desktop" && "active")} onClick={() => setDevice("desktop")} aria-label={t("Desktop preview")}><Monitor size={14} /></button><button className={clsx(device === "mobile" && "active")} onClick={() => setDevice("mobile")} aria-label={t("Mobile preview")}><Smartphone size={14} /></button><button onClick={() => setToken(null)} aria-label={t("Reset embed session")}><RefreshCcw size={13} /></button></div></header><div className={clsx("host-canvas", device === "mobile" && "mobile")}><div className="host-app"><aside><div className="host-logo">H</div><i /><i /><i /><i /></aside><main><div className="host-title"><span>{t("Record")}</span><h3>{t("Planning brief")}</h3></div><div className="host-stat-row"><div><span>{t("Status")}</span><strong>{t("Active")}</strong></div><div><span>{t("Type")}</span><strong>{t("Project")}</strong></div></div><div className="host-lines"><i /><i /><i /></div></main></div><div className="embedded-agent live-component">{token && componentReady ? createElement("alcuin-agent", { key: locale, ref: bindLiveElement, "api-url": API_URL, "session-token": token, "agent-name": agent.name, theme: "auto", lang: locale === "zh" ? "zh-CN" : "en" }) : <div className="embed-locked"><KeyRound size={20} /><strong>{t("Create a scoped session")}</strong><p>{t("The live component stays disconnected until the host obtains a short-lived token.")}</p><button onClick={() => void createSession()}>{t("Create session")}</button></div>}</div></div></section>
    </div>
