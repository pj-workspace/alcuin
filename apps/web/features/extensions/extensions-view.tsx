...
import type { Extension, ExtensionInspection, Rule, Skill } from "@alcuin/contracts";
import {
...
import { CustomizationLibrary } from "@/features/extensions/customization-library";
...
export function ExtensionsView({ extensions, skills, rules, customizationError, onChanged }: { extensions: Extension[]; skills: Skill[]; rules: Rule[]; customizationError: string | null; onChanged: () => Promise<void> }) {
  const { t } = useI18n();
  const [centerTab, setCenterTab] = useState<"connectors" | "library">("connectors");
  ...
  if (centerTab === "library") return <div className="wide-surface extensions-surface"><ExtensionCenterSwitch active={centerTab} onChange={setCenterTab} /><CustomizationLibrary skills={skills} rules={rules} error={customizationError} onChanged={onChanged} /></div>;

  return <div className="wide-surface extensions-surface">
    <ExtensionCenterSwitch active={centerTab} onChange={setCenterTab} />
    <header className="wide-header"><div><div className="eyebrow">{t("Capability registry")}</div><h1>{t("Extensions")}</h1><p>{t("Bring tools, knowledge and specialized behavior into Alcuin through explicit contracts.")}</p></div><div className="header-actions"><button className="button dark" onClick={openConnector}><Plus size={14} />{t("Connect capability")}</button></div></header>
...
function ExtensionCenterSwitch({ active, onChange }: { active: "connectors" | "library"; onChange: (tab: "connectors" | "library") => void }) {
  const { t } = useI18n();
  return <div className="extension-center-switch"><button className={clsx(active === "connectors" && "active")} onClick={() => onChange("connectors")}><Network size={14} />{t("Connections")}</button><button className={clsx(active === "library" && "active")} onClick={() => onChange("library")}><Blocks size={14} />{t("Skills & Rules")}</button></div>;
}
