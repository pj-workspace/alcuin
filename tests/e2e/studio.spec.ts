...
test("runs an approval-gated operation and updates the artifact", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("Approved operation record")).toBeVisible();
  await expect(page.getByText("The approved update was completed and recorded in the run trace.")).toBeVisible();
});
